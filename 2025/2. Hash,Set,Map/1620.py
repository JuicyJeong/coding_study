...
name_to_num = {}
num_to_name = []
num_to_name.append("") # 0부터 시작

for i in range(1, N + 1):
    name = sys.stdin.readline().strip()
    name_to_num[name] = i       # 이름으로 번호 찾기용
    num_to_name.append(name)        # 번호로 이름 찾기용
...
for _ in range(M):
    ...
    if query.isdigit():
        idx = int(query)
        results.append(num_to_name[idx])
    else: # 이름인 경우
        results.append(str(name_to_num[query]))
